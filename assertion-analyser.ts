...
function objParser(str: string, init: number = 0) {
...
function assertionAnalyser(body: any) {
  ...
  var assertions: any = splittedAssertions.slice(1);
  // match the METHODS

  var assertionBodies: any = [];
  var methods = assertions.map(function(a: any, i: number){
    var m: any = a.match(/^\s*\.\s*(\w+)__#(\d+)/);
    assertionBodies.push(parseInt(m[2]));
    var pre = splittedAssertions[i].match(/browser\s*\.\s*/) ? 'browser.' : '';
    return pre + m[1];
  });
  if(methods.some(function(m: any){ return !m })) return "invalid assertion";
  // remove parens from the assertions bodies
  var bodies = assertionBodies.map(function(b: any){
